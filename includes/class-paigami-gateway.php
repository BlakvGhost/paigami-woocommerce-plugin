...
class Paigami_WC_Gateway extends WC_Payment_Gateway {
    ...
    public function process_payment($order_id) {
        ...
        try {
            $country = sanitize_text_field($_POST['paigami_country']);
            $wallet = sanitize_text_field($_POST['paigami_wallet']);
            $phone = sanitize_text_field($_POST['paigami_phone']);
            $otp = isset($_POST['paigami_otp']) ? sanitize_text_field($_POST['paigami_otp']) : '';
            ...
            if ($response && isset($response['success']) && $response['success']) {
                ...
                $order->update_meta_data('_paigami_reference', $payment_data['reference']);
                $order->update_meta_data('_paigami_status', $payment_data['status']);
                $order->update_meta_data('_paigami_payment_url', $payment_data['payment_url'] ?? '');
                
                $order->set_transaction_id($payment_data['reference']);
                $order->save();
    ...
    private function get_currency_id($currency_code) {
        // This would need to be implemented based on your API's currency mapping
        // For now, return the currency code as is
